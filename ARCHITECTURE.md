...
flowchart LR
    subgraph aleph [Aleph Flight Computer]
        ...
        arducopter[ArduCopter Linux]
        can[CAN Bus]

        sensors --> sensorFw --> serialBridge --> elodinDb
        elodinDb --> bridge
        bridge -->|Binary sensor packets| arducopter
...
The Aleph runs the full flight stack as NixOS systemd services. Physical sensors feed through Elodin's `sensor-fw` and `serial-bridge` into **Elodin-DB**, the shared telemetry database. The **ardupilot-bridge** (Rust) subscribes to DB entities, converts sensor data to ArduPilot's binary ExternalAHRS packet protocol, and forwards motor commands back to CAN ESCs. **ArduCopter Linux** handles all flight control logic.
...
All sensors are live. The ardupilot-bridge reads from the local Elodin-DB and streams binary IMU/GPS/magnetometer packets to ArduPilot.
...
1. **IMU** (~760 Hz): `imu.gyro` / `imu.accel` -- converted to FRD rad/s and m/s², then sent as binary IMU packets.
2. **GPS** (~5 Hz): `ublox.*` -- raw UBX integers forwarded as binary GPS packets (no NED conversion in bridge).
3. **Magnetometer** (~100 Hz): `qmc5883l.mag` -- raw i16 converted to body-frame milliGauss and sent as binary mag packets.
4. **Motors** (servo rate): ArduPilot servo output parsed from UDP, written to `ardupilot.motor_command` / `ardupilot.motor_pwm`, and forwarded to CAN ESCs.
...
├── nix/
│   ├── modules/
│   │   ├── arducopter.nix           # ArduCopter Linux systemd service
│   │   ├── ardupilot-bridge.nix     # Sensor bridge systemd service
│   │   └── can.nix                  # SocketCAN interface setup
│   └── pkgs/
│       ├── arducopter.nix           # ArduCopter Linux build derivation
...
`src/ardupilot-bridge/` -- Subscribes to Elodin-DB sensor entities, assembles ArduPilot binary ExternalAHRS sensor packets, and sends them over UDP. Receives servo output from ArduPilot and writes motor telemetry back to the DB. Also forwards motor commands to CAN ESCs via SocketCAN.

Background subscription tasks run on dedicated threads for each sensor source (IMU, GPS, QMC5883L), each with independent reconnect logic.
...
The simulation writes to the same `imu` and `ublox` DB entities as real hardware. GPS noise is injected in the post-step callback using numpy PRNG (avoiding JAX JIT issues), and values are converted to UBX integer format before writing.

### ArduCopter Linux

ArduPilot's ArduCopter compiled for Linux HAL, running as a systemd service on the Aleph. Receives sensor data from the bridge via the binary ExternalAHRS backend and outputs motor commands via UDP servo packets.
